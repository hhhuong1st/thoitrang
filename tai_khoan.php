...
if (isset($_POST['dang_ky'])) {
    ...
    if (mysqli_num_rows($check_email) > 0) {
        ...
        $sql = "INSERT INTO tai_khoan (ho_ten, email, mat_khau) VALUES ('$ho_ten', '$email', '$mat_khau')";
        if (mysqli_query($conn, $sql)) {
            $thong_bao = "<p style='color: green;'>Đăng ký thành công! Vui lòng đăng nhập.</p>";
        } else {
            $thong_bao = "<p style='color: red;'>Có lỗi xảy ra, vui lòng thử lại!</p>";
        }
    }
}
...
if (isset($_POST['dang_nhap'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if (password_verify($mat_khau_nhap, $row['mat_khau'])) {
            ...
            header("Location: tai_khoan.php"); // Tải lại trang để cập nhật trạng thái
            exit();
        } else {
...
<head>
    ...
    <style>
        ...
        .box h3 { margin-top: 0; text-transform: uppercase; font-size: 20px; border-bottom: 2px solid #eee; padding-bottom: 15px; margin-bottom: 20px;}
        .form-group { margin-bottom: 15px; }
        .form-group label { display: block; margin-bottom: 8px; font-weight: bold; color: #555; }
        .form-group input { width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 4px; box-sizing: border-box; }
        .btn-submit { background: #000; color: white; padding: 12px 25px; border: none; border-radius: 4px; font-weight: bold; cursor: pointer; width: 100%; transition: 0.3s; }
